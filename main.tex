...
\paragraph{Counting arguments:}
For the problem to be even identifiable, we need
\begin{align}
    N &> \max(K\,L, K\,M) \\
    K &< \max(L, M) ~.
\end{align}
If we want the result to be stable we probably want the ``$N>$'' to become ``$N\gg$'' and the ``$K<$'' to become ``$K\ll$.''
...
The optimization proceeds by a schedule of four kinds of steps.
